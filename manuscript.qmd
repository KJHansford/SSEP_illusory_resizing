...
if (processdata==1){
  for (subj in 1:48){
    if (!dir.exists('local/EEG Data/P',subj)){dir.create('local/EEG Data/P',subj)}
  }
}

if (processdata==1){
  for (subj in 1:48){
    if (!file.exists('local/EEG Data/P',subj,'/P',subj,'_block1_EEG.fdt')){
    osfproject <- osf_retrieve_node("xc8jq")
    osffiles <- osf_ls_files(osfproject)
    fid <- which(osffiles$name=='P',subj,'_block1_EEG.fdt')
    osf_download(osffiles[fid,], path='local/EEG Data/P',subj,progress=TRUE)
}
}
}
...
if (processdata==1){
  for (subj in 1:48){
    if (!file.exists('local/EEG Data/P',subj,'/P',subj,'_block1_EEG.set')){
    osfproject <- osf_retrieve_node("xc8jq")
    osffiles <- osf_ls_files(osfproject)
    fid <- which(osffiles$name=='P',subj,'_block1_EEG.set')
    osf_download(osffiles[fid,], path='local/EEG Data/P',subj,progress=TRUE)
}
}
}
...
The stage 1 report for this study, with details regarding planned analyses, pilot data, and a Design planner (Table A1) encompassing research questions, hypotheses, sampling and analysis plans and their resulting interpretations can be seen at the following OSF page: https://osf.io/pfksu/. 
